...
// ============================================================
// 食事メニュー一括インポート（OTTO飲料メニュー編集用 - コピー.pptx 準拠）
//   既存メニューの一番下に 50 品を「追加」する。
//   ドリンクをクリアしないので、importDrinkMenu の後に1回実行してください。
// ============================================================
function importFoodMenu() {
  const sh = getSheet_(SHEET_MENU, HEADERS_MENU);

  // 既存メニューに同名食事が既にあれば二重投入しない
  const existing = getMenu().map(m => m.name);
  ...
  // 既存と重複しないものだけ追加
  const toAdd = food.filter(row => existing.indexOf(row[0]) < 0);
  if (toAdd.length === 0) {
    Logger.log('追加対象なし（全て既に登録済み）');
    return;
  }
  const startRow = sh.getLastRow() + 1;
  sh.getRange(startRow, 1, toAdd.length, 3).setValues(toAdd);
  Logger.log('食事メニュー ' + toAdd.length + ' 件を追加しました（重複スキップ ' +
             (food.length - toAdd.length) + ' 件）');
}

// ============================================================
// 全メニュー一括投入（クリア → 飲料 → 食事）
//   メニューを完全リセットしたい時に1発で全部入れ直す
// ============================================================
function importAllMenu() {
  importDrinkMenu();
  importFoodMenu();
  Logger.log('全メニュー投入完了');
}
